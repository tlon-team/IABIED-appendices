# Глава 13: Остановите это {#глава-13:-остановите-это}
...
## Часто задаваемые вопросы {#часто-задаваемые-вопросы-10}

### Можем ли мы подождать и посмотреть? {#можем-ли-мы-подождать-и-посмотреть?}

#### Нет. Мы не знаем, где находятся критические пороги. {#нет.-мы-не-знаем,-где-находятся-критические-пороги.}
...
### Будут ли предупредительные выстрелы? {#будут-ли-предупредительные-выстрелы?}

#### Возможно. Если мы хотим ими воспользоваться, готовиться надо сейчас. {#возможно.-если-мы-хотим-ими-воспользоваться,-готовиться-надо-сейчас.}
...
#### Предупреждения вряд ли будут очевидны. {#предупреждения-вряд-ли-будут-очевидны.}
...
Если завтра ИИ чуть активнее попытаются сбежать -- это не будет новостью. Следующая попытка, ещё чуть компетентнее, -- уже старая история. А когда сбежать получится -- будет уже поздно. (См. обсуждение этого явления в разделе «[Эффект Лемуана](#эффект-лемуана)» материалов к Главе 12).
...
#### «Пробуждающие» катастрофы, скорее всего, не будут вызваны суперинтеллектом. {#«пробуждающие»-катастрофы,-скорее-всего,-не-будут-вызваны-суперинтеллектом.}
...
#### Человечество так себе отвечает на сюрпризы. {#человечество-так-себе-отвечает-на-сюрпризы.}
...
#### Пора действовать. {#пора-действовать.}
...
### Как остановить *всех* без шпионского ПО на каждом компьютере? {#как-остановить-*всех*-без-шпионского-по-на-каждом-компьютере?}

#### Надо действовать быстро. {#надо-действовать-быстро.}
...
### Но вы предлагаете контролировать, сколько передовых чипов есть у частных лиц. {#но-вы-предлагаете-контролировать,-сколько-передовых-чипов-есть-у-частных-лиц.}

#### Да. А ещё мы выступаем за запрет исследований. {#да.-а-ещё-мы-выступаем-за-запрет-исследований.}
...
### Зачем запрещать исследования? Это уж слишком радикально. {#зачем-запрещать-исследования?-это-уж-слишком-радикально.}

#### Новые прорывы могут привести к тому, что создание суперинтеллекта будет невозможно остановить. {#новые-прорывы-могут-привести-к-тому,-что-создание-суперинтеллекта-будет-невозможно-остановить.}
...
### Разве можно остановить технологию? {#разве-можно-остановить-технологию?}

#### Много технологий запрещено или строго регулируется. {#много-технологий-запрещено-или-строго-регулируется.}
...
#### Запрет может быть точечным. {#запрет-может-быть-точечным.}
...
#### Значительная часть проблемы -- люди не осознают нависшую угрозу искусственного суперинтеллекта. {#значительная-часть-проблемы----люди-не-осознают-нависшую-угрозу-искусственного-суперинтеллекта.}
...
### Не даёт ли это правительствам слишком много власти? {#не-даёт-ли-это-правительствам-слишком-много-власти?}

#### У них уже есть полномочия запрещать опасные технологии. {#у-них-уже-есть-полномочия-запрещать-опасные-технологии.}
...
### А некоторые страны не откажутся от запрета? {#а-некоторые-страны-не-откажутся-от-запрета?}

#### Если они поймут угрозу, нет. {#если-они-поймут-угрозу,-нет.}
...
#### Договор потребует реального отслеживания и контроля за исполнением. {#договор-потребует-реального-отслеживания-и-контроля-за-исполнением.}
...
#### Это уже работало {#это-уже-работало}
...
### Может ли режим мониторинга длиться вечно? {#может-ли-режим-мониторинга-длиться-вечно?}

#### Нет. Понадобится какой-то другой выход из ситуации. {#нет.-понадобится-какой-то-другой-выход-из-ситуации.}
...
Настолько, чтобы они не верили в научные теории вроде аристотелизма или геоцентризма, даже если все вокруг верят. Настолько, чтобы у них был шанс пересечь пропасть между «До» и «После» с первой же попытки. (См. раздел «[„До” и „После”](#«до»-и-«после»)» материалов к Главе 10.)
...
#### Мониторингу *не следует* быть вечным. {#мониторингу-*не-следует*-быть-вечным.}
...
Это неправда. Есть и другие маршруты в столь же яркое будущее. Без риска потерять всё ни за что. Надо поискать другой путь. (См. раздел «[Если поспешить, её не будет.](#если-поспешить,-её-не-будет.)» материалов к Главе 12.)

### Зачем делать людей умнее? {#зачем-делать-людей-умнее?}

#### Это поможет решить задачу согласования. {#это-поможет-решить-задачу-согласования.}
...
Обычно такая наивность простительна. Обычно реальность не уничтожает *всё человечество* за гордыню одного алхимика. Но с машинным суперинтеллектом такой роскоши у нас нет. (См. раздел «[„До” и „После”](#«до»-и-«после»)» материалов к Главе 10.)
...
#### К усовершенствованным людям не прилагается серьезная задача «согласования человека». {#к-усовершенствованным-людям-не-прилагается-серьезная-задача-«согласования-человека».}
...
#### Работать над остановкой суперинтеллекта можно, даже если вы не согласны по поводу усовершенствования людей. {#работать-над-остановкой-суперинтеллекта-можно,-даже-если-вы-не-согласны-по-поводу-усовершенствования-людей.}
...
### «Согласованный с кем?» {#«согласованный-с-кем?»}

#### Трудный вопрос. Но разработку нужно остановить независимо от ответа. {#трудный-вопрос.-но-разработку-нужно-остановить-независимо-от-ответа.}
...
### А не разумнее ли избегать разговоров о вымирании? {#а-не-разумнее-ли-избегать-разговоров-о-вымирании?}

#### Время политических игр прошло. {#время-политических-игр-прошло.}
...
Кстати: с тех пор как мы написали предыдущий абзац, предложенная нами стратегия стала приносить всё больше результатов. См. [чуть ниже](#призн*а*ют-ли-опасность-выборные-представители-власти?), что политики говорили о суперинтеллекте летом 2025 года. Похоже, настало время для настоящего обсуждения опасности искусственного суперинтеллекта.

### Призн*а*ют ли опасность выборные представители власти? {#призн*а*ют-ли-опасность-выборные-представители-власти?}

#### Всё больше признают. {#всё-больше-признают.}
...
### Ситуация безнадёжна? {#ситуация-безнадёжна?}
...
## Расширенное обсуждение {#расширенное-обсуждение-10}

### Что нужно для глобальной остановки разработки ИИ? {#что-нужно-для-глобальной-остановки-разработки-ии?}
...
#### Предотвратить появление более многочисленных и мощных чипов для ИИ. {#предотвратить-появление-более-многочисленных-и-мощных-чипов-для-ии.}
...
#### Предотвратить использование для ИИ более мощных чипов. {#предотвратить-использование-для-ии-более-мощных-чипов.}
...
#### Предотвратить прогресс алгоритмов. {#предотвратить-прогресс-алгоритмов.}
...
Чтобы даже несовершенный запрет стал достаточно эффективным, очень важно, чтобы мировые лидеры действительно понимали -- если продолжить давить на газ, [погибнут лично они и их семьи](#а-некоторые-страны-не-откажутся-от-запрета?). Самые вероятные сценарии нарушения -- если государства решат, будто собственный суперинтеллект -- это стратегический актив (или что это иллюзия, отвлекающая от прибыльных ИИ-инструментов). Если они не будут видеть в нём кнопку для всеобщего самоубийства. Вряд ли государства станут вести секретные разработки СИИ, понимая, что это равносильно тому, чтобы зарядить пистолет, приставить к виску и спустить курок.
...
#### Чем дольше ждём, тем труднее будет {#чем-дольше-ждём,-тем-труднее-будет}
...
### Предварительный черновик договора с примечаниями {#предварительный-черновик-договора-с-примечаниями}
...
[^1]: Разберём этот пример подробнее. Кабина «Аполлона-1» загорелась во время симуляции запуска 27 января 1967 года. В NASA смогли извлечь урок из ошибки. Инженеры понимали каждую деталь ракеты. Они диагностировали проблему. Дело, вероятно, было в посеребрённом медном проводе (изоляция которого протёрлась от движения двери) рядом с подтекающей трубой охлаждения с этиленгликолем и водой. Ситуацию усугубила атмосфера из чистого кислорода в капсуле и горючие материалы в кабине. Кроме того, давление перед открытием люка надо было выровнять. Но огонь не дал добраться до управления клапаном. А пожар резко усилил разницу давления.
	...
	Современные ИИ-компании и близко не показывают такого уважения к задаче. (См. раздел «[Мы знаем, как выглядит серьёзное отношение к задаче. Тут не так.](#мы-знаем,-как-выглядит-серьёзное-отношение-к-задаче.-тут-не-так.)» материалов к Главе 11) У них нет таких подробных планов. Они не близки к правильному решению. Когда ИИ делает что-то непонятное, они не могут найти причину, как нашли тот провод. Они недостаточно близки к успеху, чтобы учиться на ошибках.
...
[^2]: Мы не рекомендуем международную коалицию для создания ИИ. (См. раздел «[Почему бы международной коалиции не разработать безопасный ИИ совместно, а не запрещать?](#почему-бы-международной-коалиции-не-разработать-безопасный-ии-совместно,-а-не-запрещать?)» материалов к Главе 12.) Но теоретически она могла бы породить организацию уровня NASA или FAA, способную реально учиться на ошибках индустрии.
